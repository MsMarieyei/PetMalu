...
Public Class ClientInfoPage
    ...
    Private Sub LoadClientInfo()
        Dim query As String = "SELECT FullName, Address, Phone, Email, Sex FROM clients WHERE ClientID = @ClientID"
    ...
    Private Sub LoadDewormingInfo()
        If String.IsNullOrWhiteSpace(TxtPetIDNo.Text) Then Return
        Dim query As String = "SELECT d.Name FROM petdewormings pd JOIN dewormings d ON pd.DewormingID = d.DewormingID WHERE pd.PetID = @PetID"
        Using conn As New MySqlConnection(connectionString)
            Using cmd As New MySqlCommand(query, conn)
                cmd.Parameters.AddWithValue("@PetID", TxtPetIDNo.Text)
                conn.Open()
                ...
                While reader.Read()
                    dewormingList.Add(reader("Name").ToString())
                End While
    ...
    Private Sub LoadVaccinationInfo()
        If String.IsNullOrWhiteSpace(TxtPetIDNo.Text) Then Return
        Dim query As String = "SELECT v.Name FROM petvaccinations pv JOIN vaccines v ON pv.VaccineID = v.VaccineID WHERE pv.PetID = @PetID"
        Using conn As New MySqlConnection(connectionString)
            Using cmd As New MySqlCommand(query, conn)
                cmd.Parameters.AddWithValue("@PetID", TxtPetIDNo.Text)
                conn.Open()
                ...
                While reader.Read()
                    vaccinationList.Add(reader("Name").ToString())
                End While
